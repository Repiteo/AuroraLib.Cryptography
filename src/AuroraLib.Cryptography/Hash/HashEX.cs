...
namespace AuroraLib.Cryptography.Hash
{
    public static class HashEX
    {
        ...
        /// <inheritdoc cref="Compute(IHash, ReadOnlySpan{char}, Encoding)"/>
        [DebuggerStepThrough]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void Compute(this IHash hash, ReadOnlySpan<char> input)
            => hash.Compute(input, Encoding.GetEncoding(28591));
        ...
        [DebuggerStepThrough]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void Compute<T>(this IHash hash, ref T input) where T : unmanaged
            => hash.Compute(input.AsBytes());
